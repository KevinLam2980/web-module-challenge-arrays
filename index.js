...
function removeFlavorByName(arr, flavour){
...
function copy(originArr, newArr){
...
console.log(copy(originalFlavors, originalFlavorsCopy));
...
function filterByWord(arr, flavString){
    let seasonalFlavs = [];
   for(let i = 0; i < arr.length; i++){
       if(arr[i].includes(flavString)){
            seasonalFlavs.push(arr[i]);
       }
   }
   return seasonalFlavs;
}
...
function getRandomFlavors(arr1, arr2, arr3, arr4){
    ...
    let randomFlavors = [];
    let randomNum = [];
    const allFlavs = [...arr1, ...arr2, ...arr3, ...arr4];
    // console.log(allFlavs);
    
    function randomNumberGen(arr){
        if(arr.length >= 31){
         return;
        } else {
        let num = Math.floor((Math.random() * allFlavs.length) + 1);
    ...
    randomNumberGen(randomNum);
    // console.log(randomNum);
    for(let i = 0; i < randomNum.length; i++){
        randomFlavors.unshift(allFlavs[randomNum[i]]);
    }
